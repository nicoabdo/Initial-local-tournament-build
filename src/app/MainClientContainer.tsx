...
export default function MainClientContainer({ initialDb }: MainClientContainerProps) {
  ...
  // Layout Tab: 'workspace' (User Prediction Workspace) | '16avos' (16 Avos Phase) | '8vos' (8 Vos Phase) | '4tos' (4 Tos Phase) | 'semifinals' (Semi finals Phase) | 'finals' (Finals Phase) | 'admin' (Admin Results Panel) | 'standings' (Family Standings) | 'statistics' (Match Analytics)
  const [activeTab, setActiveTab] = useState<"workspace" | "16avos" | "8vos" | "4tos" | "semifinals" | "finals" | "admin" | "standings" | "statistics">("workspace");
        ...
        <main className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className={`${activeTab === "admin" ? "lg:col-span-2" : "lg:col-span-3"} flex flex-col gap-8`}>
            ...
            {activeTab === "4tos" && (
               <PredictionWorkspace
                 users={db.users}
                 matches={db.matches.filter(m => m.phase === "4 tos")}
                 pointStructure={db.settings.pointStructure}
                 unsavedChanges={unsavedChanges}
                 onPredChange={handlePredChange}
                 onSavePredictions={handleSavePredictions}
             ...
             {activeTab === "finals" && (
                <PredictionWorkspace
                  users={db.users}
                  matches={db.matches.filter(m => m.phase === "Finals")}
                  pointStructure={db.settings.pointStructure}
                  unsavedChanges={unsavedChanges}
                  onPredChange={handlePredChange}
                  onSavePredictions={handleSavePredictions}
                  ...
                  isSaving={isSaving}
                  saveSuccess={saveSuccess}
                  title="Finals Predictions Grid"
                  subTitle="View & edit all participant predictions for the Finals phase"
                />
              )}
            {activeTab === "admin" && (
              <AdminResultsPanel
