...
const nicoData = {
  ...
  "betting_scores_extension": [
    { "matchId": 13, "predicted_home": 5, "predicted_away": 0 },
    ...
    { "matchId": 35, "predicted_home": 2, "predicted_away": 1 },
...
let user = db.users.find(u => u.id === nicoData.id);
if (!user) {
  // If Nico doesn't exist, create him
  user = {
    id: nicoData.id,
    name: nicoData.name,
    total_points: 0,
...
// Map predictions from Nico's payload
nicoData.betting_scores_extension.forEach(pred => {
...
// Run point recalculation for all users (including Nico)
db.users.forEach(u => {
  let userTotal = 0;
...
console.log('Successfully added NICO predictions and recalculated standings.');
