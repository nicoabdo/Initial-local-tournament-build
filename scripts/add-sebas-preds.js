...
const sebasData = {
  ...
  "betting_scores_extension": [
    ...
    { "matchId": 28, "predicted_home": 2, "predicted_away": 2 },
    { "matchId": 29, "predicted_home": 4, "predicted_away": 2 },
...
let user = db.users.find(u => u.id === sebasData.id);
if (!user) {
  user = {
    id: sebasData.id,
    name: sebasData.name,
...
// Map predictions from Sebas's payload
sebasData.betting_scores_extension.forEach(pred => {
...
// Run point recalculation for all users (including Sebas)
db.users.forEach(u => {
  let userTotal = 0;
...
console.log('Successfully added SEBAS predictions and recalculated standings.');
