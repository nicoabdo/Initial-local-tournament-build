...
const leoData = {
  ...
  "betting_scores_extension": [
    ...
    { "matchId": 16, "predicted_home": 1, "predicted_away": 2 },
...
let user = db.users.find(u => u.id === leoData.id);
if (!user) {
  user = {
    id: leoData.id,
    name: leoData.name,
...
// Map predictions from Leo's payload
leoData.betting_scores_extension.forEach(pred => {
...
// Run point recalculation for all users (including Leo)
db.users.forEach(u => {
  let userTotal = 0;
...
console.log('Successfully added LEO predictions and recalculated standings.');
