...
const sarahData = {
  ...
  "betting_scores_extension": [
    ...
    { "matchId": 15, "predicted_home": 0, "predicted_away": 2 },
    { "matchId": 16, "predicted_home": 1, "predicted_away": 0 },
    { "matchId": 17, "predicted_home": 2, "predicted_away": 0 },
...
let user = db.users.find(u => u.id === sarahData.id);
if (!user) {
  user = {
    id: sarahData.id,
    name: sarahData.name,
...
// Map predictions from Sarah's payload
sarahData.betting_scores_extension.forEach(pred => {
...
// Run point recalculation for all users (including Sarah)
db.users.forEach(u => {
  let userTotal = 0;
...
console.log('Successfully added SARAH predictions and recalculated standings.');
