...
// 1. Load the original official scores before the purge
console.log('Retrieving original match scores from previous commit...');
let matchScores = {};
...
try {
  const oldDbJson = execSync('git show 97cdeed:db.json', { encoding: 'utf-8' });
  const oldDb = JSON.parse(oldDbJson);
  
  if (oldDb.settings) {
    originalSettings = oldDb.settings;
  }
  
  if (oldDb.matches) {
    oldDb.matches.forEach(m => {
      // Only keep scores that were actually entered (status finished/live)
  ...
  console.log(`Successfully retrieved scores for ${Object.keys(matchScores).length} matches.`);
} catch (err) {
  console.error('Failed to get match scores from previous commit. Defaulting to empty.', err);
}
...
data.forEach((row, idx) => {
  ...
  // Preserve official scores if present
  let actualHome = null;
  let actualAway = null;
  let status = 'scheduled';
  
  if (matchScores[matchIdStr]) {
    actualHome = matchScores[matchIdStr].actual_home_score;
    actualAway = matchScores[matchIdStr].actual_away_score;
    status = matchScores[matchIdStr].status;
  }
...
console.log('Recalculating tournament standings points...');
function recalculatePoints(db) {
...
recalculatePoints(fullDatabase);

// Write files
fs.writeFileSync(DB_FILE, JSON.stringify(fullDatabase, null, 2), 'utf-8');
console.log('SUCCESS: db.json updated successfully with official scores preserved.');
