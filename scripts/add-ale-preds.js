...
const aleData = {
  ...
  "betting_scores_extension": [
    ...
    { "matchId": 23, "predicted_home": 2, "predicted_away": 0 },
    { "matchId": 24, "predicted_home": 0, "predicted_away": 3 },
...
let user = db.users.find(u => u.id === aleData.id);
if (!user) {
  user = {
    id: aleData.id,
    name: aleData.name,
...
// Map predictions from Ale's payload
aleData.betting_scores_extension.forEach(pred => {
...
// Run point recalculation for all users (including Ale)
db.users.forEach(u => {
  let userTotal = 0;
...
console.log('Successfully added ALE predictions and recalculated standings.');
