...
const mariData = {
  ...
  "betting_scores_extension": [
    ...
    { "matchId": 19, "predicted_home": 2, "predicted_away": 1 },
    ...
    { "matchId": 32, "predicted_home": 1, "predicted_away": 0 },
    ...
    { "matchId": 36, "predicted_home": 1, "predicted_away": 2 }
  ]
};
...
let user = db.users.find(u => u.id === mariData.id);
if (!user) {
  user = {
    id: mariData.id,
    name: mariData.name,
...
// Map predictions from Mari's payload
mariData.betting_scores_extension.forEach(pred => {
...
// Run point recalculation for all users (including Mari)
db.users.forEach(u => {
  let userTotal = 0;
...
console.log('Successfully added MARI predictions and recalculated standings.');
